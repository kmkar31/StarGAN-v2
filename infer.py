...
class Model:
    def __init__(self, save_path):
        ...
        self.generator.load_state_dict(torch.load(os.path.join(
            save_path, "generator.pth"), map_location=torch.device('cpu')))
        self.mapping_network.load_state_dict(torch.load(os.path.join(
            save_path, "mapping_network.pth"), map_location=torch.device('cpu')))
        self.style_encoder.load_state_dict(torch.load(os.path.join(
            save_path, "style_encoder.pth"), map_location=torch.device('cpu')))
        self.face_mask.load_state_dict(torch.load(os.path.join(
            save_path, "facemask.pth"), map_location=torch.device('cpu')))
    ...
    def get_style(self, x, y):
        ...
        if len(x.shape) == 4:
            ...
            raise ValueError(
                "Incorect shape (possible latent space/reference image)")
...
def create_interpolation_video(model, src_imgs, ref_imgs):
    ...
    alpha = [0] + [sigmoid(alpha) for alpha in np.arange(-5,
                                                         5, 0.5) if alpha not in [-1, 0, 1]] + [1]
    count = 0
    frames = []
    for i in range(len(ref_imgs)-1):
        ...
        for a in alpha:
            ...
            wb[:, :, int(round(256*(1-a))):256, :] = ref_now[:,
                                                             :, 0:int(round(256*a)), :]
            top_row = torch.cat([wb, src], dim=0)

            wb = torch.ones(1, 3, 256, 256)
            wb[:, :, 0:int(round(256*(1-a))), :] = ref_now[:,
                                                           :, int(round(256*a)):256, :]
            wb[:, :, int(round(256*(1-a))):256, :] = ref_next[:,
                                                              :, 0:int(round(256*a)), :]
            ...
            if a == 0:
                for _ in range(10):
                    frames.append(Image.open(
                        "frames/{}.png".format(count)).convert('RGB'))
            frames.append(Image.open(
                "frames/{}.png".format(count)).convert('RGB'))
            count += 1

    frames[0].save('out.gif', save_all=True, optimize=False,
...
if __name__ == "__main__":
    ...
    imgs = sorted(glob(os.path.join("celeba_hq/val/", "male", '*.jpg'))) + \
        sorted(glob(os.path.join("celeba_hq/val/", "female", '*.jpg')))
    src_imgs = random.choices(imgs, k=5)
    ref_imgs = random.choices(imgs, k=10)

    create_interpolation_video(model, src_imgs, ref_imgs)

    # ref = load_img("../celeba_hq/val/male/008550.jpg")
    # src = load_img("src.jpeg")
