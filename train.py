...
# Number of images in each class are not the same
...
class Trainer:
    def __init__(self, config):
        ...
        if not os.path.exists(self.output_dir):  # Make an output directory
        ...
        self.generator = starganv2.Generator(
            self.img_size, self.style_dim).to(self.device)
        self.mapping_network = starganv2.MappingNetwork(
            self.latent_dim, self.style_dim, self.num_domains).to(self.device)
        self.style_encoder = starganv2.StyleEncoder(
            self.img_size, self.style_dim, self.num_domains).to(self.device)
        self.discriminator = starganv2.Discriminator(
            self.img_size, self.num_domains).to(self.device)

        self.gen_optim = optim.Adam(params=self.generator.parameters(), lr=self.lr, betas=[
                                    self.beta1, self.beta2], weight_decay=self.weight_decay)
        self.map_optim = optim.Adam(params=self.mapping_network.parameters(
        ), lr=self.map_lr, betas=[self.beta1, self.beta2], weight_decay=self.weight_decay)
        self.style_optim = optim.Adam(params=self.style_encoder.parameters(
        ), lr=self.lr, betas=[self.beta1, self.beta2], weight_decay=self.weight_decay)
        self.disc_optim = optim.Adam(params=self.discriminator.parameters(
        ), lr=self.lr, betas=[self.beta1, self.beta2], weight_decay=self.weight_decay)

        train_dir = os.path.join(self.root_dir, "train")
        train_dataset = dataloader.CelebA(
            train_dir, self.config, transform=True)
        ...
        self.train_dataloader = DataLoader(train_dataset, batch_size=self.batch_size,
                                           num_workers=self.num_workers, drop_last=True, pin_memory=True, sampler=sampler)
    ...
    def train(self):
        ...
        for indx, (src, src_label, ref1, ref2, ref_label, latent1, latent2) in enumerate(self.train_dataloader):
            ...
            src, src_label, ref1, ref2, ref_label, latent1, latent2 = src.to(self.device), src_label.to(self.device), ref1.to(
                self.device), ref2.to(self.device), ref_label.to(self.device), latent1.to(self.device), latent2.to(self.device)
            ...
            loss = loss_adv + self.lambda_style*loss_style - \
                self.lambda_div*loss_div + self.lambda_cyc*loss_cyc
            self._reset_grad()
            loss.backward()
            self.gen_optim.step()
            self.map_optim.step()
    ...
    def _translate_and_rec(self, src, src_label, ref, ref_label):
        ...
        self._save_image(imgs, N, os.path.join(self.output_dir, "tr_and_rec", "{}.png".format(
            self.step)), "translate and reconstruct", self.step)
    ...
    def _translate_with_ref(self, src, ref, ref_label):
        ...
        self._save_image(imgs, N+1, os.path.join(self.output_dir, "tr_with_ref",
                                                 "{}.png".format(self.step)), "translate with reference", self.step)
    ...
    def _translate_with_lat(self, src, ref_labels, latent_refs, psi):
        ...
        for i, ref_label in enumerate(ref_labels):
            ...
            labels_many = torch.LongTensor(10000).to(
                self.device).fill_(ref_label[0])
        ...
        imgs = torch.cat(imgs, dim=0)
        self._save_image(imgs, N, os.path.join(self.output_dir, "tr_with_lat", "{}_{}.png".format(
            self.step, psi)), "translate with latent {}".format(psi), self.step)
    ...
    def debug(self):
        ...
        random_ref_labels = [torch.tensor(y).repeat(N).to(
            self.device) for y in range(self.num_domains)]
        random_latent = torch.randn(
            self.num_domains, 1, self.latent_dim).repeat(1, N, 1).to(self.device)
        for psi in [0.5, 0.7, 1.0]:
            # translate with different latent vectors
            self._translate_with_lat(
                srcs, random_ref_labels, random_latent, psi)
    ...
    def r1_reg(self, logits, inp):
        ...
        grad_dout = torch.autograd.grad(outputs=logits.sum(
        ), inputs=inp, create_graph=True, retain_graph=True, only_inputs=True)[0]
